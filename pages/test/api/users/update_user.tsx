...
export const UpdateUserPage: NextPage = ({ }: ListPageProps) => {
  ...
  const addUser = async () => {
    ...
    const myAxios = axios.create({
      baseURL: '/api/v1/users/update_user',
      method: 'POST',
      timeout: 3600,
    ...
    then(res => {
      ...
      setOldNameError(reason.response?.data?.errors?.oldNameError);
  ...
  return (
    <>
      <form onSubmit={onSubmit}>
        ...
        />
        {oldNameError &&
          <span style={{color: 'red'}}>{oldNameError}</span>
        }
        <br/>
        <input
          type="text"
